...
type Tx struct{
	...
	Sender		ed25519.PublicKey				//Sender and signer of transaction public key that is a one time use just for this trasnaction
	...
	Approver	ed25519.PublicKey				//PublicKey of Approver  Must be from an authorized approver for this bundle
...
//setup map of transactions
	Transactions		map[uint64]Tx					//Known Transactions
	notValid  			map[uint64][]uint64				//Map of potential transactions ID to node IDs indicating a bad transaction was approved. We do not track valid transaction in a map that is track in each bundle, but this is a way to track and vote a bad transaction be reversed and removed.
